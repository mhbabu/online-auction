...
class ProductTypeDataTable extends DataTable
{
    ...
    public function ajax()
    {
        return datatables()
            ...
            ->addColumn('action', function ($data) {
                $actionBtn = '<a href="/admin/product/types/' . Encryption::encodeId($data->id) . '/edit/" class="btn btn-sm btn-primary" title="Edit Product Category"><i class="fa fa-edit"></i> Edit</a> ';
                $actionBtn .= '<a href="/admin/product/types/' . Encryption::encodeId($data->id) . '/delete/" class="btn btn-sm btn-danger action-delete" title="Delete Product Category"><i class="fa fa-trash"></i> Delete</a> ';
                return $actionBtn;
            })
    ...
    {
        $query = ProductType::getProductTypeList();
        $data = $query->select([
            'product_types.*'
        ]);
